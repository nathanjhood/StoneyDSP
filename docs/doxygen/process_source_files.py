...
def remove_stoneydsp_namespaces(source):
    """Return a string of source code with any StoneyDSP namespaces removed.
    """
    namespace_regex = re.compile(r"\s+namespace\s+StoneyDSP\s*{")
...
def add_doxygen_group(path, group_name):
    """Add a Doxygen group to the file at 'path'.
       ...
       The addition of stoneydsp namespacing code to all of the source files breaks
       backwards compatibility by changing the doc URLs, so we need to remove
       the namespaces.
    """

    filename = os.path.basename(path)
    if re.match(r"^stoneydsp_.*\.(h|dox)", filename):
        with open(path, "r") as f:
            content = f.read()
        with open(path, "w") as f:
            f.write("\r\n/** @weakgroup " + group_name + "\r\n *  @{\r\n */\r\n")
            f.write(remove_stoneydsp_namespaces(content))
            f.write("\r\n/** @}*/\r\n")
...
if __name__ == "__main__":
    ...
    # Get the list of StoneyDSP modules to include.
    if args.subdirs:
        stoneydsp_modules = args.subdirs.split(",")
    else:
        stoneydsp_modules = []
        for item in sorted(os.listdir(args.source_dir)):
            if os.path.isdir(os.path.join(args.source_dir, item)):
                stoneydsp_modules.append(item)

    # Copy the StoneyDSP modules to the temporary directory, and process the source
    # files.
    module_definitions = []
    for module_name in stoneydsp_modules:
    ...
    with open(os.path.join(args.dest_dir, "stoneydsp_modules.dox"), "w") as f:
        f.write("\r\n\r\n".join(module_definitions))

    # Copy markdown docs - note that JUCE's docs also adds
    # "JUCE Module Format.md" to this array,
    for name in ["CMake API.md"]:
